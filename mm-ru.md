...
использовалось сжатие, а `JSON` структуры сериализировалось с помощью `MessagePack`.
...
сжатием и отправкой телеметрии на сервер. Это решение показало себя гораздо лучше. Н
о так ведь быть не должно?! Или должно?
Данные между процессами посылались через `pipe`, они сериализировалось на одной стороне и десериализовались на другой.
...
Этот случай один из многих, которые показывают, что однопоточные корутины не решают всех проблем. А так же
...
Переключение между задачами в корутинах стоит копейки по сравнению с переключением контекста между
потоками на уровне ОС. А `CPU-bound` задачи получают настоящий параллелизм на разных ядрах процессора —
каждый worker грузит своё ядро на 100%, не мешая соседям.
...
**race conditions** невозможны. Да, `workers` работают параллельно, но если следовать принципу
Shared Nothing, то проблемы синхронизации остаются локализованными.
...
все это необходимо для эффективной работы с горутинами и каналами.
...
и кодом логики для `worker pool`. Ручное управление даёт возможность 
выжать теоретический максимум из оборудования и кода. Но с другой стороны это так же является недостатком.

### Недостатки модели

**1. Ручное распределение нагрузки**

Ручное распределение нагрузки — палка о двух концах. С одной стороны, программист может оптимизировать
код под конкретные задачи, с другой может ошибиться и неправильно оценить, что должно идти в `IO-bound` код,
а что в `workers`. В результате `IO-bound` код может быть перегружен тяжёлыми задачами, 
что приведёт к деградации отзывчивости сервера, увеличения латентности.

Модель требует от программистов PHP был достаточно квалифицированными, 
или же полагаться на других квалифицированных программистов.
...
Возможно, вы скажите: с этим можно жить! Мы готовы! Но готов ли сам PHP стать многопоточным? 
...
Или ещё лучше: зачем она **НЕ НУЖНА**. 

> Многопоточность не нужна для параллельного выполнения кода.

Идея о том, что многопоточность нужна для параллельного выполнения кода, 
находится глубоко в сознании программистов,
точно так же, как идея о том, что чёрные дыры "засасывают" материю.
...
а факт их завершения можно отслеживать с помощью сигналов (событий ОС). Зачем тогда нужны потоки?

Чтобы дать честный ответ на этот вопрос, придётся отправиться в далёкое прошлое и призвать 
известных людей, чтобы они могли лично ответить на вопросы и попытаться объяснить принятые решения. 
...
> Потоки нужны, чтобы передавать память между задачами без дополнительных инструментов
...
Итак, потоки нужны, чтобы эффективно передавать данные между задачами. 
Что так же верно, как и то, что "чёрные дыры не сосут".

## Модель памяти PHP

А как работает память в PHP?
Абстрактную модель памяти PHP, можно описать следующим образом:
1. Код
2. Данные 
3. Состояние PHP VM

Код `PHP` может быть разделён между потоками уже сейчас (задача была решена с появлением `PHP JIT`).
...
PHP использует **Thread-Local Storage (TLS)** для хранения состояния виртуальной машины в каждом потоке. 
Это критически важный механизм, который обеспечивает изоляцию состояния между потоками в ZTS-режиме (Zend Thread Safety).

Чтобы получить указатель на состояние VM в современных сборках PHP, используется 
объявление "статической" переменной по стандурту C11 `__thread` (или `__declspec(thread)` в MSVC). 
Скорость такой операции максимально высока, и на `x86_64` архитектуре сводится к чтению из регистра `FS` или `GS`.
...
благодаря кастомной кодогенерации в ассемблер, передавая состояние VM через регистры `CPU`.
Для `PHP` такой трюк невозможен, так как под капотом он использует `С`-функции, 
а в `С` не существует способа указать контекст как особый параметр, который передаётся всем функциям скрытно. 
...
Например, `PHP Fiber` копируют часть указателей на глобальные структуры (`zend_executor_globals`) при переключении.
...
2. PH VM private. Часть виртуальной машины, которая должна быть уникальной.

![PHP VM shared vs private](diagrams/tls-globals-structure.svg)

Часть из этих структур может быть помечена как `shared`, а часть как `private`, и даже 
`Executor Globals` можно разделить на `shared` и `private` части и тем самым добиться эффективного 
перемещения состояния VM между потоками. Глобальные структуры расширений не потеряют производительность,
из-за косвенного доступа, так как они уже его используют.
...
Именно эти особенности мешают эффективно разделить состояние VM на `shared` и `private` части.
...
существует в Rust и других языках. Такой способ передачи памяти между потоками обладает следующими преймуществами:
1. Безопасность. Только один поток владеет объектом
2. Отсутствие накладных расходов на копирование или сериализацию

Однако у этого способа есть проблемы. Рассмотрим дерево категорий:
...
Здесь можно придумать несколько вариантов алгоритма, который бы можно было назвать `deep copy`.
...
По сравнению с сериализацией это более быстрая версия копирования, так как она не требует
преобразования данных в транспортный формат и обратно. 
Кроме того, есть возможность создать гибридный алгоритм, 
который будет перемещать данные с `refcount` = 1, и запускать `deep copy` для других типов объектов.
...
Звучит хорошо, потому что никаких изменений в сам язык `PHP` вносить не требуется, 
а многопоточность становится доступной.

Однако на уровне ядра `PHP` не всё так замечательно. Чтобы перемещение объекта стало реальностью, 
...
1. Менеджер памяти потока `B` видит, что адрес объекта ему не известен. 
...
* он увеличивает время обращения к данным, свойствам
...
добавляет **+34% overhead** к изолированной операции `refcount++`. 
...
Такой подход решает половину проблемы, вторая половина проблема состоит в разработке механизма сборки мусора для такого объекта. 
...
к имплиментации Акторов, специальных объектов,
...
потоке могут существовать множество корутин, ожидающие выполнения тяжёлых задач. 
При этом поток, обслуживающий будет сохранять способность быстро реагировать на новые запросы, 
...
будет страдать от проблем с производительностью и утечками памяти, от циклов сборки мусора.
...
Позитивный момент состоит в том, что текущая реализации Garbage Collector `PHP` 
...
но требует от программиста внимания, квалификации и усилий. Но существует
...
он не требует изменений в синтаксисе языка и ограничено вписывается в существующую философию ООП.

Пример напоминает обычный последовательный код, который выполняется внутри корутины. 
Но так как методы `postMessage` и `getMessages` выполняться в другом потоке, на самом деле 
...
* либо принадлежат только этому актору, выделены в его уникальном region.
* либо принадлежат и перемещены из других регионов или потоков
...
которая раскручивается `Scheduler`. Именно `Scheduler` распределяет время процессора 
...
На сегодняшний день в рамках проекта `TrueAsync` уже реализована 
экспериментальная версия многопоточного менеджера памяти и API для создания потоков. 
Объём работы, нужный для рефакторинга состояния PHP VM, 
чтобы позволить безопасно перемещать его приватную часть между потоками, выглядит реалистичным и выполнимым.

Можно с уверенностью сказать, что многопоточный безопасный и надёжный PHP — не безумная мечта, а достижимая цель.