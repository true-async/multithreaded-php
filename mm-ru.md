...
```asm
      ; offset - постоянное смещение, расчитывается в момент компиляции
...
2. `PH VM` movable. Часть виртуальной машины, которая должна быть перемещаемой.
...
Всё это, конечно, хорошо, но когда же мы сможем это увидеть в реальности? Скажите вы.

Модель `Single-threaded + offload` имеет все шансы появиться ближайшее время, так как множество 
...
И являются реалистичной достойной целью для `PHP 9`, предлагая рынку безопасный многопоточный язык программирования.